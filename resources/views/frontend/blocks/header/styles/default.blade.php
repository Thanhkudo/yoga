...
<header class="site-navbar js-sticky-header site-navbar-target" role="banner">
    <div class="container">
        <div class="row align-items-center position-relative">
            <div class="site-logo">
                <a href="{{ route('frontend.home') }}" class="text-black"><img
                    ...
                    <ul class="site-menu main-menu js-clone-nav ml-auto d-none d-lg-block">
                        @isset($main_menu)
                            @foreach ($menu as $item)
                                @if ($item->parent_id == $main_menu->id)
                                    @if ($item->sub <= 0)
                                        <li class="nav-item {{ $item->url_link == url()->full() ? 'active' : '' }}">
                                            <a class="nav-link text-uppercase f-w-600" href="{{ $item->url_link }}"
                                                title="{{ $item->name ?? '' }}">
                                                {{ $item->name ?? '' }}
                                            </a>
                                        </li>
                                    @else
                                        <li
                                            class="has-children nav-item {{ $item->url_link == url()->full() ? 'active' : '' }}">
                                            <a class="a-img caret-down" href="{{ $item->url_link }}"
                                                title="{{ $item->name ?? '' }}">
                                                {{ $item->name ?? '' }}
                                            </a>
                                            ...
                                            <ul class="dropdown arrow-top">
                                                @foreach ($menu as $item_sub)
                                                    @if ($item_sub->parent_id == $item->id)
                                                        @if ($item_sub->sub <= 0)
                                                            <li class="">
                                                                <a class="" href="{{ $item_sub->url_link }}"
                                                                    title="{{ $item_sub->name ?? '' }}">
                                                                    {{ $item_sub->name ?? '' }}
                                                                </a>
                                                            </li>
                                                        @else
                                                            <li class="has-children">
                                                                <a class="caret-down" href="{{ $item_sub->url_link }}"
                                                                    title="{{ $item_sub->name ?? '' }}">
                                                                    {{ $item_sub->name ?? '' }}
                                                                </a>
                                                                    ...
                                                                    @foreach ($menu as $item_sub_2)
                                                                        @if ($item_sub_2->parent_id == $item_sub->id)
                                                                            @if ($item_sub_2->sub <= 0)
                                                                                <li class="">
                                                                                    <a class=""
                                                                                        href="{{ $item_sub_2->url_link }}"
                                                                                        title="{{ $item_sub_2->name ?? '' }}">
                                                                                        {{ $item_sub_2->name ?? '' }}
                                                                                    </a>
                                                                                </li>
                                                                            @else
                        ...
                        <li>
                            <div class="d-lg-flex nav-link">
                                <a href="tel:{{ $web_information->information->phone }}" class="nav-link"><button
                                        class="btn btn-success bg_green text-uppercase bora-30 text-uppercase">
                                        @lang('Đăng ký học thử')
                                    </button></a>
                                <div class="text-left">
                                    <p class="f-w-500">
                                        ...
                                        {{ $web_information->information->appointment }}
                                    </p>
                                    <p>
                                        <svg class="mr-2" width="16" height="23" viewBox="0 0 16 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <span
                                            class="red f-w-500">{{ $web_information->information->phone }}</span>
                                        @lang('For Appointment')
                                    </p>
                                </div>
                            </div>
                        </li>
